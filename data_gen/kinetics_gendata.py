...
class Feeder_kinetics(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with open(sample_path, 'r') as f:   #'r'表示读文件
...
def gendata(data_path, label_path,
    ...
    fp = open_memmap(
        ...
        dtype='float32',
        mode='w+',
        shape=(len(sample_name), 3, max_frame, num_joint, num_person_out))

    # fp = np.zeros((len(sample_name), 3, max_frame,num_joint , num_person_out), dtype=np.float32)  #返回给定形状（N,C,T,V,M）和类型(float32)的新数组，用0填充

    for i, s in enumerate(tqdm(sample_name)):    #i索引值，s元素
        data, label = feeder[i]
        fp[i, :, 0:data.shape[1], :, :] = data
        sample_label.append(label)

    with open(label_out_path, 'wb') as f:  #以二进制格式打开一个文件只用于写入。如果该文件已存在则将其覆盖。如果该文件不存在，创建新文件。
        pickle.dump((sample_name, list(sample_label)), f) #pickle使用转换二进制的数据压缩方法保存数据
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--data_path', default='E:/project/datasets/kinetics-skeleton/kinetics-skeleton')
    parser.add_argument(
